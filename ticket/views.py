...
from ticket.forms import IssueCreationForm, AddUpdate, AssignComment
from ticket.filters import IssuesFilter
from ticket.tables import IssueTable
...
class CreateIssue(View):
    def get(self, request):
        ...
        return render(request, 'ticket/createIssue.html', {'form': form, 'message': 'Issue Form'})
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            issue.save()
        return render(request, 'ticket/createIssue.html', {'message': 'Issue Created sucessfuly!'})


class TicketAddUpdate(View):
    def get(self, request, ticket):
        form = AddUpdate()
        return render(request, 'ticket/addUpdate.html', {'form': form})
    ...
    def post(self, request, ticket):
        ...
        if form.is_valid():
            ...
            update.save()
        return HttpResponseRedirect(reverse('ticket:ticket', args=[ticket]))
...
class AssignmentsList(ListView):
    ...
    template_name = 'ticket/assigment.html'
    paginate_by = 1
...
class TicketsAssignComment(CreateView):
    ...
    template_name = 'ticket/assignComment.html'
    ...
    def get_success_url(self):
        return reverse('ticket:ticket', args=[self.kwargs['ticket']])


class FilteredIssueListView(SingleTableMixin, FilterView):
    table_class = IssueTable
    model = Issue
    template_name = "ticket/issue.html"
